...
│   └── shared/
│       └── Assets.ts            # Asset keys and file paths (both Assets and AssetPaths)
...
|---|---|
| A new interactable type (NPC, door, chest...) | Create `features/objects/behaviors/MyThingInteractable.ts` extending `Interactable`. Implement `onInteract()`. Add its key to `Assets.ts` and its path to `AssetPaths.ts`, then load it in the scene's `preload()`. |
| A new scene | Create `core/scenes/MyScene.ts` (extend `GameScene` if the player walks around in it). Add a key to `core/config/SceneKeys.ts` and register the scene class in `Main.ts`. |
| A new asset (image) | Drop the file in `public/` (use the matching subfolder like `player/`, `objects/`, `world/`, or `scenes/`), add a key in `Assets.ts` and a path in `AssetPaths.ts`, then load it in the scene that uses it. |
